...
import { fetchCollaboratorSettings, updateCollaboratorSettings } from "../../fetch/fetchSettings";
...
export const SettingScreenCol = ({ type = 1 }: { type: number }) => {
...
const GeneralSettings = ({ loading = false }: { type: number, loading?: boolean }) => {
    console.log(loading)
    const dispatch: storeDispatchType = useDispatch()
    const userInfo: any = useSelector((state: storeStateType) => state.collaboratorInfo)
    const accType = userInfo.user?.collaborator?.accountType
    ...
    return (<div className={`flex flex-col p-4 gap-8 sm:gap-10 sm:p-8 overflow-y-scroll overflow-x-hidden justify-start items-center rounded-2xl border border-secondaryLight w-full scroll-smooth scrollbar-thin dark:scrollbar-track-primary  dark:scrollbar-thumb-accent h-[95%] `}>
        <div className="flex flex-col gap-8 sm:gap-10 w-fit justify-center items-center" >
            <ProfileImageUploader key={userInfo.user?.profilepic} setNewProfilepicFile={setNewProfilepicFile} accountType={userInfo.user?.collaborator?.accountType} imgUrl={userInfo.user?.profilepic} />
            ...
            <SettingsToggle value={userInfo.user?.Collaborator?.whatsAppNotifcation} label="Whatsapp" type={SettingsToggleType.Whatsapp} />
...
const SettingsToggle: React.FC<{ value: boolean | undefined, label: string, type?: SettingsToggleType }> = ({ label, value }) => {
    ...
    const toggle = () => {
        ...
        if (label === "Whatsapp")
            dispatch(collaboratorActions.updateCollaboratorInfo({ whatsAppNotifcation: !value }))
        else if (label === "Email")
            dispatch(collaboratorActions.updateCollaboratorInfo({ emailNotifcation: !value }))
        else if (label === "Push Notification")
            dispatch(collaboratorActions.updateCollaboratorInfo({ pushNotifcation: !value }))
...
export const SettingsFields: React.FC<{ setNewPassword?: (...args: any) => void, currentName: string | null | undefined, type: settingsFiledsType, label: string }> = ({ type, label, currentName, setNewPassword }) => {
    ...
    const handleSave = () => {
        setTxt(editingTxt);
        dispatch(collaboratorActions.updateUserInfo({ username: editingTxt }))
        setIsEditing(false)
    }
...
const SettingsArea = ({ type }: { type: number }) => {
    ...
    const fetchFnc = useCallback(() => fetchCollaboratorSettings(username as string), [username])
    ...
    useEffect(() => {
        if (userInfoData) {
            dispatch(collaboratorActions.setUserInfo({ user: userInfoData }))
